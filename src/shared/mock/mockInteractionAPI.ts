...
export const mockInteractionApi = (interactionNodes: InteractionNode[]): ProfileInteractionV2Api => {
  ...
  return {
    ...
    getChatState: (id) => {
      ...
      if (!interaction || interaction.type !== 'chat') return Promise.resolve(null)
    ...
    getNavState: (id) => {
      ...
      if (!interaction || interaction.type !== 'nav') return Promise.resolve(null)
      const { title, description, favIconUrl, imageAssetId, updatedAt } = interaction
      return Promise.resolve({ title, description, favIconUrl, imageAssetId, updatedAt })
    },

    getChats: ({ contextId, created, updated, limit, model, order }) => {
      return Promise.resolve(
        interactions
          .filter(interaction => 
            ...
            (model === undefined || (interaction as ChatInteraction).model === model)
          )
          .sort((a, b) => order === 'asc' ? a.createdAt - b.createdAt : b.createdAt - a.createdAt)
          .slice(-limit) as ChatInteraction[]
      )
    },

    getChatIds: ({ contextId, created, updated, limit, model, order }) => {
      return Promise.resolve(
        interactions
          .filter(interaction =>
            interaction.type === 'chat' &&
    ...
    appendAssistantContent: async (id: number, content: string, timestamp: number) => {
      const interaction = interactionMap.get(id) as ChatInteraction
...
export type SkeletonNode = { id: number, type: 'chat' | 'nav', children?: SkeletonNode[] }

export const buildMockNodes = (skeletons: SkeletonNode[]): InteractionNode[] => {
  ...
  const mapSkeleton = (skeleton: SkeletonNode): InteractionNode => {
    ...
    const node: InteractionNode = skeleton.type === 'chat' ? {
      id: skeleton.id,
      type: 'chat',
      userContent: faker.lorem.sentence(),
      createdAt: timestamp,
      model: faker.helpers.arrayElement(['gpt-4o', 'gpt-4o-mini', 'deepseek-chat', 'deepseek-reasoner']),
